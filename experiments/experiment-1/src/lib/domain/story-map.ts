...
/** Reorders an Activity within the map's backbone. */
export function moveActivity(
	...
	findActivity(map, activityId);
	const siblings = map.activities.filter((a) => a.id !== activityId);
	const rank = resolveRank(siblings, beforeId, afterId, `map ${map.id}`);
	return {
		...map,
		activities: map.activities
			.map((a) => (a.id === activityId ? { ...a, rank } : a))
			.sort((a, b) => (a.rank < b.rank ? -1 : a.rank > b.rank ? 1 : 0))
	};
}

/** Reorders a Step within its current activity. */
export function reorderStep(
	...
	const activity = activityOwning(map, stepId);
	return moveStep(map, stepId, activity.id, beforeId, afterId);
}

/** Moves a Step to a (possibly different) Activity, carrying its Stories.
 * The stories' `sliceId` values are untouched — slice membership is
 * orthogonal to which activity owns the step. */
export function moveStep(
	...
	const fromActivity = activityOwning(map, stepId);
	const toActivity = findActivity(map, toActivityId);
	const step = fromActivity.steps.find((s) => s.id === stepId)!;

	const targetSiblings = (
		fromActivity.id === toActivityId
			? toActivity.steps.filter((s) => s.id !== stepId)
			: toActivity.steps
	).map((s) => ({ id: s.id, rank: s.rank }));
	const rank = resolveRank(targetSiblings, beforeId, afterId, `activity ${toActivityId}`);

	const movedStep: Step = { ...step, activityId: toActivityId, rank };

	const activities = map.activities.map((a) => {
		if (a.id === fromActivity.id && a.id === toActivityId) {
			return { ...a, steps: [...a.steps.filter((s) => s.id !== stepId), movedStep] };
		}
		if (a.id === fromActivity.id) {
			return { ...a, steps: a.steps.filter((s) => s.id !== stepId) };
		}
		if (a.id === toActivityId) {
			return { ...a, steps: [...a.steps, movedStep] };
		}
		return a;
	});
