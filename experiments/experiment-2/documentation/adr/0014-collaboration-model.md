# 0014: Collaboration model — server-authoritative commands, SSE fan-out, notify-and-refetch

## Status

Accepted, 2026-09-03. **Carried over from experiment-1's ADR 0015**, which superseded that experiment's ADR 0014 —
not reproduced here — and which
...
**Extended by [ADR 0015](./0015-accounts-sessions-and-map-membership.md), 2026-09-03**,
...
> **Amended for experiment-2, 2026-09-04.** This ADR is carried over as written, because
> almost none of it was about SQLite: the hub, the SSE transport, notify-and-refetch,
> presence, cursors and the reconnection buffer are pure TypeScript and moved across
> untouched. Three passages below are now historical rather than current, and are left in
> place rather than edited, so the reasoning still reads as it was made:
>
> - **Stage 0's SQLite mechanics.** WAL, `busy_timeout` and `SQLITE_BUSY` have no
>   equivalent. What replaced them is [ADR 0016](./0016-optimistic-concurrency-by-document-cas.md):
>   a single conditional update, atomic on its own, with no transaction mode to depend on.
>   The version round-trip and the per-map `KeyedLock` are unchanged — neither was ever
>   about SQLite.
> - **"One server, one SQLite file."** Now one server, one MongoDB database
>   ([ADR 0003](./0003-mongodb-one-document-per-map.md)). The single-process assumption the
>   hub and the lock rest on is unchanged, and is still the thing that would break first
>   under a second instance.
> - **§2's correction about contending "at the SQLite level."** The property it names —
>   that no two writers compute ranks against the same state — still holds, now because the
>   compare-and-set refuses the second writer outright.

## Context

experiment-1's ADR 0014 established the decision (collaboration is in scope, not yet built) and the
...
built. The first contradicts experiment-1's ADR 0014 and is the more important of the two.

### There is no working concurrency control today

experiment-1's ADR 0014 states that two editors touching different cards conflict, and that the second is
...
experiment-1's ADR 0014's cost list is therefore wrong in one direction and right in the other: the
...
- It requires **no domain changes at all**. experiment-1's ADR 0014's `{ map, change }` migration stays
...
> **Superseded by ADR 0015.** The anonymous cookie is not built. Real accounts landed
...
**The `{ map, change }` refactor experiment-1's ADR 0014 declined stays declined.** Stage 1 delivers live
...
One correction to experiment-1's ADR 0014's estimate, found while designing: effects would **not** require
