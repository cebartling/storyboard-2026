# 0015: Accounts, sessions and map membership — the port-signature change ADR 0006 priced

## Status

Accepted, 2026-09-03. **Extends [ADR 0014](./0014-collaboration-model.md)** — which put
...
built. ADR 0014 designs concurrent _editing_ — several browsers on one map — and is careful
...
may edit them. Without that, ADR 0014's presence layer would put real-looking names on
...
nothing the runtime does not already have. And persistence is one MongoDB database (ADR 0003)
...
An `owner_id` column alone would have satisfied finding A10 and made ADR 0014 pointless —
...
in-memory double and the MongoDB adapter must pass: a rule that is not in that file is
...
routes and the hook), already testable against an in-memory MongoDB. Under ADR 0006's own
...
### 6. ADR 0014's anonymous id is withdrawn; presence becomes `{ userId, displayName, clientId }`
...
**What this buys.** A map is visible to the people on it and to nobody else. ADR 0014's
