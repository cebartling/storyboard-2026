...
import { addActivity, addSlice, addStep, addStory, createMap, dialog, firstStepId } from './board';
import { beat, openBrowser, openWindow, say, startServer } from './harness';

/**
 * A guided walkthrough of collaboration and accounts, driven as two real browser
 * windows side by side. Each step captions itself on the page, so the demo
 * explains what you are looking at as it happens.
 *
 *   corepack pnpm demo
 *
 * This is not a test. It asserts only enough to keep itself honest and to wait
 * for the right moments; it is slow on purpose, and it is deliberately outside
 * both suites.
 */
...
async function walkthrough() {
	...
	try {
		// Side by side, so both halves of every exchange are visible at once.
		const { page: alice } = await openWindow(browser, { left: 0, top: 0, width, height });
		...
		await say(
			bob,
			'Bob',
			`Registered as a separate account, in a separate browser profile.`,
			'good'
		);
		await beat();
		...
		await say(
			alice,
			'Alice',
			'Alice edits a different card entirely, moving the board on.',
			'warn'
		);
		...
		await say(
			alice,
			'Alice',
			'Alice, the owner, still has Delete — and the map is intact.',
			'good'
		);
		await alice.goto('/');
